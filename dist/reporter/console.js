...
export function reportConsole(result, explain) {
    if (explain) {
...
function reportExplain(result) {
    ...
    for (const fn of result.functions) {
        ...
        for (const call of fn.detectedSDKCalls) {
            const hasPerm = fn.effectivePermissions.length === 0 ||
                fn.effectivePermissions.some((p) => p.actions.some((a) => matchActionSimple(a, call.action)));
            if (hasPerm) {
...
function reportSummary(result) {
    console.log();
    console.log(`Analyzed ${result.functions.length} function(s), found ${result.violations.length} issue(s)`);
    console.log();
    reportViolations(result.violations);
}
function reportViolations(violations) {
    ...
    console.log(`Found ${violations.length} issue(s): ${errors} error(s), ${warnings} warning(s)`);
    console.log();
...
function matchActionSimple(pattern, action) {
    ...
    if (pa === "*")
        return true;
    if (pa?.endsWith("*"))
        return aa?.startsWith(pa.slice(0, -1)) ?? false;
    return false;
}
