...
export interface SDKPattern {
  service: string;
  commandName: string; // SDK v3 Command class (e.g., SendEmailCommand)
  requiredActions: string[];
  resourceType: string; // High-level category (S3, DynamoDB, etc.)
}

/** All known AWS SDK v3 command -> IAM action mappings. */
export const AWS_SDK_PATTERNS: SDKPattern[] = [
  // SES
...
const commandMap = new Map<string, SDKPattern>();
for (const p of AWS_SDK_PATTERNS) {
  commandMap.set(p.commandName, p);
}

/** Look up an SDK pattern by command class name. */
export function lookupCommand(commandName: string): SDKPattern | undefined {
